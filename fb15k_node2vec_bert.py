...
ent2text_path = 'data/FB15K/entity2textlong.txt'
...
# print(rel2id)
ent2text = {}
with open(ent2text_path, 'r', encoding='utf-8') as f:
    data = f.readlines()
    for k in data:
        d = k.strip('\n').split('\t')
        ent2text[d[0]] = d[1]
...
#     if node in ent2text:
#         sent_embedding = bert_model.encode(ent2text[node])
#         node_bert = np.add(embedding, sent_embedding)
        # index = node_embeddings.get_index(node)
        # node_embeddings[index] = node_bert
        # node_bert_embeddings[node] = node_bert
        # print(f"Node: {node}, bert + Embedding: {node_bert}")
        # print(f"Node: {node}, bert + Embedding 1: {node_embeddings[index]}")
    # print(node_embeddings)
# # #
# # k = m.wv
# # print(k)
# with open(f'data/FB15k_query2tree/fb15k_bert_kg_node2vec.pkl', 'wb') as file:
#     pickle.dump(node_embeddings, file)

# m.wv.save_word2vec_format(f'data/FB15k_query2tree/fb15k_bert_kg_node2vec.wv')
# m.save(f'data/FB15k_query2tree/fb15k_bert_kg_node2vec.model')

with open('data/FB15k_query2tree/fb15k_bert_kg_node2vec.pkl', 'rb') as f:
...
# nn = neigh.fit(data)
# with open(f'data/FB15k_query2tree/fb15k_brute_bert.pkl', 'wb') as kdfile:
#       pickle.dump(nn,kdfile)
# kdtree = KDTree(data)
# with open(f'data/FB15k_query2tree/fb15k_kd_tree_bert.pkl', 'wb') as kdfile:
...
# with open('data/FB15k_query2tree/fb15k_kd_tree_bert.pkl', 'rb') as file:
#     kdtree = pickle.load(file)

with open('data/FB15k_query2tree/fb15k_brute_bert.pkl', 'rb') as file:
    nn = pickle.load(file)
...
# '''
q2b_path = 'Q2Bdata/FB15k/test_ans_ci.pkl'
...
for k, v in test_1_p.items():
    ...
    #
    # two_project = set()
    # for p in ind.tolist()[0][1:]:
    #     d_p = G.degree[id2ent[str(p)]]4
    #     i = nn.kneighbors(np.reshape(node_embeddings.get_vector(p), (-1, 384)), n_neighbors=d_p, return_distance=False)
    #     two_project = two_project.union(set(i.tolist()[0][:1]))
    # tree = two_project
    tree0 = ind.tolist()[0][1:]
    # tree_0 = ind0.tolist()[0][1:]
    tree1= ind1.tolist()[0][1:]
    # tree1_neg = set(ent_v).difference(set(tree1))
    tree2 = ind2.tolist()[0][1:]
    tree2_neg = set(ent_v).difference(set(tree2))
    # tree_1_2 = set(tree1).union(set(tree2))
    # tree_0_1 = set(tree_0).union(tree_1_2)
    tree = (set(tree0).union(set(tree1))).union(tree2_neg)
    # print(tree)
    rr = []
    for a in list(v):
        try:
...
#
e = time.time() - s
print(e/len(test_1_p))
print(f'MRR: {np.mean(mrr)}')
with open('data/FB15k_query2tree/fb15k_bert_pin.pkl', 'wb') as f:
