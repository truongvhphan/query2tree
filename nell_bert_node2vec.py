...
dev_path = 'dev.txt'
test_path = 'test.txt'
ent2text_path = 'entity2textlong.csv'
...
with open(ent2id_path, 'r', encoding='utf-8') as f:
  ...
  for d in data:
      ...
      id2ent[int(d[1])] = d[0]
      ent.append(int(d[1]))
    # if i >= 60000 and i < 80000:
    #     ent2id[d[0]] = int(d[1])
    #     id2ent[int(d[1])] = d[0]
    #     text = ' '.join(d[0].split('_')[2:])
    #
    #     try:
    #         content = wikipedia.summary(text)
    #         print(i, '---', content)
    #         ent2text[d[0]] = content
    #         f = open('entity2textlong.csv', 'a', encoding='utf-8', newline='')
    #         w = csv.writer(f, delimiter='\t')
    #         w.writerow([d[0], content])
    #         f.close()
    #     except:
    #         ent2text[d[0]] = 'unknown'
    #         print(i, '--- unknown')
    #         f = open('entity2textlong.csv', 'a', encoding='utf-8', newline='')
    #         w = csv.writer(f, delimiter='\t')
    #         w.writerow([d[0], 'unknown'])
    #         f.close()
    #     i +=1
    # else:
    #     i +=1
...
# with open('entity2textlong.pkl', 'wb') as f:
#     pickle.dump(ent2text, f)
...
# with open(f'nell_bert_kg_node2vec.pkl', 'wb') as file:
#     pickle.dump(node_bert_embeddings, file)

with open(f'nell_bert_kg_node2vec.pkl', 'rb') as file:
...
# for node in G.nodes:
    ...
    # except:
    #     text = ' '.join(node.split('_')[2:])
    #     try:
    #         content = wikipedia.summary(text)
    #         print(i, '---', content)
    #         embedding = bert_model.encode(content)
    #         data.append(embedding)
    #         ent2text[node] = embedding
    #         node_embeddings[node] = embedding
    #     except:
    #         ent2text[node] = 'unknown'
...
# nn = neigh.fit(data)
# with open(f'nell_bert_brute.pkl', 'wb') as kdfile:
...
with open('nell_bert_brute.pkl', 'rb') as file:
    nn = pickle.load(file)
...
for k, v in test_1_p.items():
    ...
    ind = nn.kneighbors(np.reshape(node_embeddings[ent], (-1,384)), n_neighbors=deg, return_distance=False)
    # # # # ind0 = nn.kneighbors(np.reshape(node_embeddings.get_vector(ent0), (-1, 384)), n_neighbors=deg0, return_distance=False)
    ind1 = nn.kneighbors(np.reshape(node_embeddings[ent1], (-1, 384)), n_neighbors=deg1, return_distance=False)
    ind2 = nn.kneighbors(np.reshape(node_embeddings[ent2], (-1,384)), n_neighbors=deg2, return_distance=False)
    # # # #
    # # #
    # tree = ind.tolist()[0][1:]
    ...
    tree1_neg = set(ent).difference(set(tree1))
    tree2 = ind2.tolist()[0][1:]
    tree2_neg = set(ent).difference(set(tree2))
...
with open('nell_bert_pin.pkl', 'wb') as f:
